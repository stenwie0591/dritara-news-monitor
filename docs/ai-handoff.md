...
- Evidenze: clean install del lock dev riuscita in venv temporanea Python 3.13; `pip-audit` runtime senza vulnerabilità note; `pip check` verde; `make check` e clean-env suite verdi con 228 test. `python-dotenv` è passato da 1.0.1 a 1.2.2 per chiudere PYSEC-2026-2270. Il primo run CI ha trovato `greenlet` condizionale su Linux x86_64/aarch64, assente nella risoluzione macOS ARM: ora è un pin diretto e la matrice corretta 3.11/3.13 va confermata post-push.
