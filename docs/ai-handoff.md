...
- Evidenze: clean install del lock dev riuscita in venv temporanea Python 3.13; `pip-audit` runtime senza vulnerabilità note; `pip check` verde; `make check` e clean-env suite verdi con 228 test. `python-dotenv` è passato da 1.0.1 a 1.2.2 per chiudere PYSEC-2026-2270. La CI ha rilevato `greenlet` condizionale Linux, ora pin diretto, e test scorer accidentalmente dipendenti dalla directory DB locale, ora isolati in-memory; la matrice corretta 3.11/3.13 va confermata post-push.
